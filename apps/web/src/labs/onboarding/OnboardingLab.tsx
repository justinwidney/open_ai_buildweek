...
/**
 * The production trailhead only needs the two values that establish month
 * zero. The larger profile builder remains available to the isolated lab.
 */
export function JourneyKickoffExperience({ initialProfile, onComplete }: JourneyKickoffExperienceProps) {
  ...
  function begin(event: FormEvent) {
    ...
    if (!Number.isInteger(startingAge) || startingAge < 16 || startingAge > 100) {
      setError("Enter an age between 16 and 100.");
      return;
    }
    if (!Number.isFinite(startingNetWorth) || Math.abs(startingNetWorth) > 100_000_000) {
      setError("Enter a net worth between -$100 million and $100 million.");
      return;
    }
    ...
    const profile: JourneyOnboardingProfile = {
      ...
      workExperience: {
        ...
        grossHourlyRateCents: null,
        grossPayPerPeriodCents: null,
        hasBonusOrCommission: false,
        notes: null,
      },
    };
      ...
      <div className="onboarding-layout onboarding-layout--kickoff">
        <aside className="onboarding-rail scroll">
          ...
          <div className="onboarding-rail__intro">
            <span className="eyebrow">Before the first crossroads</span>
            <h1>Choose your starting point.</h1>
            <p>Two details place you on the map and establish your finances before your first decision.</p>
          </div>
    ...
    <main className="onboarding-shell">
      ...
      <div className="onboarding-layout">
        <aside className="onboarding-rail scroll" aria-label="Onboarding progress">
          <div className="onboarding-brand"><div className="crest" aria-hidden="true">♜</div><div><span>Conquer Your Path</span><b>Life journey</b></div></div>
